        ...
        if 'register_doctor' in request.POST:
            ...
            try:
                send_user_email(
                    ...
                    message=f"Dear {request.user.username},\n\nYou have successfully registered for the doctor: {doctor.name}. Please complete the payment to finalize your registration.\n\nBest regards,\nYour Platform Team",
        ...
        if 'register_tutor' in request.POST:
            ...
            try:
                send_user_email(
                    ...
                    message=f"Dear {request.user.username},\n\nYou have successfully registered for the tutor: {tutor.name}. Please complete the payment to finalize your registration.\n\nBest regards,\nYour Platform Team",
                    recipient_email=request.user.email
                )
            except Exception as e:
                messages.error(request, f"Registration successful, but failed to send email: {str(e)}")
    ...
    if request.method == 'POST':
        if profile:  # Ensure the profile exists before modifying it
            ...
            if 'cancel_doctor' in request.POST:
                # Store the doctor name before removing
                doctor_name = doctor.name if doctor else None
                profile.doctor = None
                profile.save()
                doctor = None  # Update local variable

                # Send cancellation email for doctor
                if doctor_name:
                    try:
                        send_user_email(
                            subject="Doctor Registration Cancellation",
                            message=f"Dear {request.user.username},\n\nYou have successfully cancelled your registration for the doctor: {doctor_name}.\n\nBest regards,\nYour Platform Team",
                            recipient_email=request.user.email
                        )
                    except Exception as e:
                        messages.error(request, f"Registration cancelled, but failed to send email: {str(e)}")

            # Cancel Tutor Registration
            if 'cancel_tutor' in request.POST:
                # Store the tutor name before removing
                tutor_name = tutor.name if tutor else None
                profile.tutor = None
                profile.save()
                tutor = None  # Update local variable

                # Send cancellation email for tutor
                if tutor_name:
                    try:
                        send_user_email(
                            subject="Tutor Registration Cancellation",
                            message=f"Dear {request.user.username},\n\nYou have successfully cancelled your registration for the tutor: {tutor_name}.\n\nBest regards,\nYour Platform Team",
                            recipient_email=request.user.email
                        )
                    except Exception as e:
                        messages.error(request, f"Registration cancelled, but failed to send email: {str(e)}")
    ...
    try:
        ...
        try:
            send_user_email(
                ...
                message=f"Dear {request.user.username},\n\nYour payment of ₹10 has been successfully processed. Your registration is now complete.\n\nBest regards,\nYour Platform Team",
                recipient_email=request.user.email
            )
        except Exception as e:
            messages.error(request, f"Payment successful, but failed to send email: {str(e)}")
